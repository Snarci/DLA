...
class ResidualBlock50(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, downsample=False):
        ...
        self.downsample = downsample
        if self.downsample:
            self.downsample_conv = nn.Conv2d(in_channels, out_channels * 4, kernel_size=1, stride=stride)
            self.downsample_bn = nn.BatchNorm2d(out_channels * 4)
        self.attention = CBAMBlock(out_channels * 4)
...
class ResNet50(nn.Module):
    ...
    def _make_layer(self, in_channels, out_channels, num_residual_blocks, stride=1):
        layers = []
        layers.append(ResidualBlock50(in_channels, out_channels, stride, downsample=True))
        for _ in range(num_residual_blocks - 1):
            layers.append(ResidualBlock50(out_channels, out_channels))
        return nn.Sequential(*layers)
